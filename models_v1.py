...
def train(model, model_name, var, version="v1", total_timesteps=10000):
    ...
    model.save(f"models/{version}/{model_name}_{var}")
...
def plot_metrics(metrics_dict: Dict[str, Dict[str, list]], version="v1", var=0):
    ...
    plt.savefig(f'plots/{version}/eval_rewards_{var}.png')
    plt.close()
    ...
    for i, v in enumerate(eval_lengths_arr):
        plt.text(i, v + 0.5, f'{v:.2f}', ha='center')
    plt.savefig(f'plots/{version}/eval_lengths_{var}.png')
    plt.close()
    ...
    plt.legend()
    plt.savefig(f'plots/{version}/training_rewards_{var}.png')
    plt.close()
...
def main(var, acts, obs, env_configs):
    ...
    if os.path.exists(f'models/{version}/metrics_{var}.pkl'):
        print("Loading metrics from pickle file")
        with open(f'models/{version}/metrics_{var}.pkl', 'rb') as f:
    ...
    with open(f'models/{version}/metrics_{var}.pkl', 'wb') as f:  # open a text file
